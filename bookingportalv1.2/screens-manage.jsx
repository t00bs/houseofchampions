...
function ScreenEvents() {
  const upcoming = [
    ["12", "JUN", "Thu · 18:30", "Founders' Supper Club", "Dinner · Thought Lounge · with the Council", "Members", "ok"],
    ["17", "JUN", "Tue · 08:00", "Dawn Pages — a writers' breakfast", "Workshop · Launch Pad", "A few places", "low"],
    ["21", "JUN", "Sat · 19:00", "Long Table: Midsummer", "Social · the floor", "Waitlist", "full"],
    ["25", "JUN", "Wed · 12:30", "Lunch & Learn — Raising in Jersey", "Talk · Future Room", "Members", "ok"],
  ];
  const past = [
    ["Thu 29 May", "Print Night — risograph & negronis", "Spark Studio"],
    ["Wed 21 May", "The Concierge Sessions Nº 03", "Thought Lounge"],
    ["Fri 16 May", "Spark Studio Live — podcast night", "Spark Studio"],
  ];
  return (
    <DesktopShell active="events">
      <div className="wf-col" style={{ gap: 20, height: '100%' }}>
        <PageHead eyebrow="Nº 006 / Events" title="What's on at the House." size={30}
          sub="The House events programme — suppers, talks, workshops and the occasional long table. Tickets and RSVPs are handled on Eventbrite."
          right={<Btn arrow>View all on Eventbrite</Btn>} />

        <Label>Upcoming</Label>
        <div className="wf-col" style={{ gap: 12 }}>
          {upcoming.map((e, i) => (
            <div key={i} className="wf-card" style={{ padding: 16, display: 'flex', alignItems: 'center', gap: 18 }}>
              <div className="wf-col" style={{ alignItems: 'center', gap: 2, flex: '0 0 54px', borderRight: '1px solid var(--wf-line)', paddingRight: 16 }}>
                <span className="wf-serif" style={{ fontSize: 26, fontWeight: 600, lineHeight: 1 }}>{e[0]}</span>
                <Meta style={{ fontSize: 9 }}>{e[1]}</Meta>
              </div>
              <Ph h={54} w={72} caption="" />
              <div className="wf-col" style={{ gap: 5, flex: 1 }}>
                <Meta style={{ color: 'var(--wf-ink)' }}>{e[2]}</Meta>
                <Title size={18}>{e[3]}</Title>
                <span style={{ fontSize: 12, color: 'var(--wf-ink-3)' }}>{e[4]}</span>
              </div>
              <Tag variant={e[6] === 'ok' ? 'ok' : (e[6] === 'full' ? '' : 'acc')}>{e[5]}</Tag>
              <Btn sm arrow>View on Eventbrite</Btn>
            </div>
          ))}
        </div>
        ...
        <div className="wf-col" style={{ gap: 12 }}>
          ...
          {past.map((e, i) => (
            <div key={i} className="wf-row" style={{ padding: '12px 18px', border: '1px solid var(--wf-line)', alignItems: 'center', gap: 16, opacity: .7 }}>
              <Meta style={{ flex: '0 0 110px' }}>{e[0].toUpperCase()}</Meta>
              <span style={{ flex: 1, fontSize: 13, color: 'var(--wf-ink-2)' }}>{e[1]}</span>
              <span style={{ fontSize: 12, color: 'var(--wf-ink-3)' }}>{e[2]}</span>
              <Btn sm arrow>View on Eventbrite</Btn>
            </div>
          ))}
        </div>

        <Note kind="Eventbrite">Events are managed in Eventbrite. Each “View on Eventbrite” opens the listing in a new tab for tickets, RSVP and full detail — no in-app ticketing in this phase.</Note>
      </div>
    </DesktopShell>
  );
}
      ...
      <div style={{ position: 'absolute', inset: 0, background: 'rgba(30,28,24,.4)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div className="wf-card" style={{ width: 460, padding: 28, background: 'var(--wf-panel)', boxShadow: '0 24px 80px rgba(0,0,0,.3)' }}>
          ...
          <div className="wf-card flat" style={{ border: '1px solid var(--wf-line)', background: 'var(--wf-panel-2)', padding: 14, marginBottom: 20 }}>
            <div className="wf-row" style={{ justifyContent: 'space-between' }}><Label>Allowance returned</Label><span className="wf-meta" style={{ color: 'var(--wf-ok)' }}>+2 hrs to your quarter</span></div>
    ...
    <DesktopShell active="bookings">
      <div style={{ maxWidth: 560, margin: '0 auto', width: '100%', paddingTop: 20, textAlign: 'center' }}>
        <div className="wf-col" style={{ alignItems: 'center', gap: 14 }}>
          ...
          <Lead style={{ maxWidth: 400 }}>Your Future Room reservation for Thursday 5 June has been released, and 2 hours are back in your quarterly boardroom allowance.</Lead>
        </div>
        <div className="wf-card" style={{ padding: 18, marginTop: 22, textAlign: 'left' }}>
          <div className="wf-row" style={{ justifyContent: 'space-between', alignItems: 'center' }}>
            <Allow label="Boardroom hours" used={4} total={8} unit=" hrs" note="2 hrs returned" style={{ flex: 1 }} />
          </div>
        </div>
