...
function ScreenAdminPlans() {
  const tiers = ["Atelier", "Studio", "House", "Collective"];
  const groups = [
    ["Booking allowances", [["Desk days / month", ["Flexible", "Flexible", "Unlimited", "Unlimited"]], ["Meeting-room hrs / month", ["4", "8", "Unlimited", "Unlimited"]], ["Events / month", ["4", "2", "Unlimited", "Unlimited"]]]],
    ["Accessible spaces", [["Top Floor desks", ["✓", "✓", "✓", "✓"]], ["2nd Floor (quiet)", ["—", "✓", "✓", "✓"]], ["Future Room", ["Rate", "Rate", "Incl.", "Incl."]]]],
    ["Time windows", [["Booking window", ["7 days", "14 days", "30 days", "30 days"]], ["Out-of-hours access", ["—", "—", "✓", "✓"]]]],
    ["Guests & sharing", [["Guests / month", ["1", "2", "4", "Agreed"]], ["Shared seat", ["—", "✓", "—", "—"]]]],
  ];
  return (
      ...
      <div className="wf-col" style={{ gap: 20 }}>
        ...
        <div className="wf-card" style={{ padding: 0, overflow: 'hidden' }}>
          <div className="wf-row" style={{ borderBottom: '1px solid var(--wf-line)', background: 'var(--wf-panel-2)', padding: '14px 20px', alignItems: 'flex-end' }}>
            ...
            {tiers.map((t, i) => <div key={i} className="wf-col" style={{ flex: 1, gap: 3, alignItems: 'flex-start' }}><span className="wf-serif" style={{ fontSize: 17, fontStyle: 'italic' }}>{t}</span><Meta style={{ fontSize: 8.5 }}>{["£225", "£275", "£400", "INVITE"][i]}</Meta></div>)}
          </div>
        ...
        <div className="wf-row" style={{ gap: 14 }}>
          ...
          <Note kind="Extensible">“+ Add a tier” clones a column to seed a new tier (e.g. a future day-pass or student plan) without touching the rest.</Note>
        </div>
      </div>
    </AdminShell>
  );
...
function ScreenAdminMembers() {
  const rows = [
    ...
    ["Amara Vance", "House", "Individual", "Active", "Nov 2025", "ok"],
    ["Reuben Okafor", "House", "Individual", "Suspended", "Dec 2025", "susp"],
        ...
        <div className="wf-row" style={{ gap: 18, alignItems: 'flex-start' }}>
          ...
          <div className="wf-col" style={{ flex: '0 0 320px', gap: 16 }}>
            <div className="wf-card" style={{ padding: 18, borderColor: 'var(--wf-accent)' }}>
              ...
              <div className="wf-col" style={{ gap: 14 }}>
                <Allow label="Desk days" used={3} total={5} />
                <Allow label="Room hours" used={6} total={8} unit=" hrs" warn />
              </div>
            </div>
            <div className="wf-card" style={{ padding: 18 }}>
